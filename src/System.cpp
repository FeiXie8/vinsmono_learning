    ...
    :bStart_backend(true)
{
    ...
    // string sConfig_file = sConfig_file_ + "simu_config.yaml";
...
// TODO:homework
void System::PubPointData(double dStampSec, std::vector<cv::Point2f> &feature)
{
    if (!init_feature)
    {
    ...
    if (PUB_THIS_FRAME)  // 用仿真数据时，此部分应该是连续的，也就是一直为true，否则会漂移
    {
        pub_count++;
        ...
        for(int i=0; i<NUM_OF_CAM; i++)
        {
            ...
            for(int j=0; j<feature.size(); j++)
            {
                // if(trackerData[i].track_cnt[j] > 1)
                {
                    ...
                    double x = feature[j].x;
                    double y = feature[j].y;
                    double z = 1;
            ...
            if(!init_pub)
            {
                cout << "4 PubImage init_pub skip the first imag!" << endl;
                init_pub = 1;
            }
            else
            {
    ...
    while (bStart_backend)
    {
        ...
        for (auto &measurement : measurements)
        {
            ...
            if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
            {
                ...
                save_Pose("keyframePose.txt", dStamp, p_wi, q_wi);
...
void System::save_Pose(std::string filename, double time, Eigen::Vector3d p, Eigen::Quaterniond q)
{

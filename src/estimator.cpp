...
//TODO 这里pre_integrations存储着两帧之间的预积分值，为什么又要重新算一遍？
...
void Estimator::MargOldFrame()
{
    ...
    //这里每个时刻的cams和VB是放在一起的，最后15个维度是新的帧的Pose和VB
    for (int i = 0; i < WINDOW_SIZE + 1; i++)
    {
    ...
    //Marginalize函数会计算先验信息，estimate会一直维持着最新的先验信息
